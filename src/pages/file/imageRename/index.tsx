  ...
  useEffect(() => {
    if (mapping && sourcePath) {
      window.FileService.folderList(sourcePath).then((files) => {
        ...
        const nextSourceFiles: string[] = [];
        const nextTargetFiles: string[] = [];

        for (const [from, to] of map) {
          console.log({ from, to })
          files.forEach((file) => {
            if (file.filePath.match(new RegExp(`^${from}[ \\.\\-\\(\\[（【].*$`,'i'))) {
              nextSourceFiles.push(file.filePath);
              nextTargetFiles.push(file.filePath.replace(new RegExp(`^${from}([ \\.\\-\\(\\[（【].*)$`,'i'), `${to}$1`));
            }
          })
        }
        setSourceFiles(nextSourceFiles.map((file) => ({ name: file })));
        setTargetFiles(nextTargetFiles.map((file) => ({ name: file, status: ImageFlattenStatus.UNKNOW })));
      })
    }
  }, [mapping, sourcePath]);
